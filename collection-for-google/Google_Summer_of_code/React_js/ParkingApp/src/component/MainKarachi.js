...
import { reservedOne , reEditOne , getResultOne } from '../store/action/action'
import swal from 'sweetalert';
import './main.css'

class MainKarachi extends Component {
    constructor(props){
        super(props)
        this.state = {text :{
            message : '',
                ...
                <div className="row">
                    ...
                    <div className="col-md-12 mb-5">
                        <div className="text-center container">
                            <div className="row">
                                { this.props.karachiVal.map( ( val , index )=>{
                                    return(
...
function mapStateToProp(state) {
    return ({
        karachiVal: state.root.karachiVal,
    })
}
function mapDispatchToProp(dispatch) {
    return ({
        reserved: (index , uid , editor) => {
            dispatch(reservedOne(index , uid , editor));
        },
        reEdit: (index , uid , editor) => {
            dispatch(reEditOne(index , uid , editor));
        },
        getResult: () => {
            dispatch(getResultOne());
        }
    })
}

export default connect(mapStateToProp, mapDispatchToProp)(MainKarachi);