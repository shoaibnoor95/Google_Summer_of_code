...
import { reservedTwo , reEditTwo , getResultTwo } from '../store/action/action'
import swal from 'sweetalert';
import './main.css'

class MainLondon extends Component {
    constructor(props){
        super(props)
        this.state = {text :{
            message : '',
                ...
                <div className="row">
                    ...
                    <div className="col-md-12 mb-5">
                        <div className="text-center container">
                            <div className="row">
                                { this.props.londonVal.map( ( val , index )=>{
                                    return(
...
function mapStateToProp(state) {
    return ({
        londonVal: state.root.londonVal,
    })
}
function mapDispatchToProp(dispatch) {
    return ({
        reserved: (index , uid , editor) => {
            dispatch(reservedTwo(index , uid , editor));
        },
        reEdit: (index , uid , editor) => {
            dispatch(reEditTwo(index , uid , editor));
        },
        getResult: () => {
            dispatch(getResultTwo());
        }
    })
}

export default connect(mapStateToProp, mapDispatchToProp)(MainLondon);